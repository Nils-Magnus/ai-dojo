...
def download(key):
    server = "http://files.point-8.de"
    remote_dir = "trainings/ds101"
    data_dir = "../.assets/data"
    dataset_path = f"{data_dir}/{key}"
    if os.path.isdir(dataset_path):
        print(f"{dataset_path} already exists")
        return
    # download .zip file
    remote_path = f"{server}/{remote_dir}/{key}.zip"
    print(f"downloading from {remote_path} to {data_dir}")
    subprocess.run(
        [
            "wget",
            f"--directory-prefix={data_dir}",
            remote_path
        ],
        check=True
    )
    # extract
    print(f"unzipping {data_dir}/{key}.zip")
    with zipfile.ZipFile(f"{data_dir}/{key}.zip", "r") as zip_file:
...
def read_chicago_taxi_trips_daily(data_path="../.assets/data/taxi/taxi_trips_daily.csv"):
...
def read_house_prices(
    ...
    if encode_categorial:
        data_cat = pandas.get_dummies(
            data[categorial_selected],
            drop_first=drop_first_level
        )
        data = pandas.concat(
            [
...
def read_heartbeat(
    data_path="../.assets/data/Heartbeat 2",
    balance_classes=False
    ):
    """
    Read the heartbeat ECG time series dataset, labelled as normal/data_abnormal
    Source: https://www.kaggle.com/shayanfazeli/heartbeat/
    """
    data_normal = pandas.read_csv(
        f"{data_path}/ptbdb_normal.csv",
        header=None
    )
    data_abnormal = pandas.read_csv(
        f"{data_path}/ptbdb_abnormal.csv",
        header=None
    )
    if balance_classes:
        data_abnormal = data_abnormal.sample(n=data_normal.shape[0])
    ...
    data, label = raw_data[raw_data.columns[:-1]], raw_data[raw_data.columns[-1]]
